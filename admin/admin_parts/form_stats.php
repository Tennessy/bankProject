...
<form method="post" action="admin.php?action=stats" name="form_stats_nbContracts" id="form_stats_nbContracts" class="form_admin">
	<fieldset>
		...
		<p> <label for="input_stats_nbContracts_dateB">Date de début :</label>
			<input type="text" name="input_stats_nbContracts_dateB" required="required" placeholder="JJ/MM/AAAA" value="" id="input_stats_nbContracts_dateB" />
		</p>
		<p> <label for="input_stats_nbContracts_dateB">Date de fin :</label>
			<input type="text" name="input_stats_nbContracts_dateE" required="required" placeholder="JJ/MM/AAAA" value="" id="input_stats_nbContracts_dateE" />
		</p>
		<?php
		if (
			...
			if ($queryDB) {
				...
				echo '<p>Nombre total de contrats entre ' . $_POST['input_stats_nbContracts_dateB'] . ' et ' . $_POST['input_stats_nbContracts_dateE'] . ' : ' . $nbContracts . '.</p>';
			}
		}
		?>
...
<form method="post" action="admin.php?action=stats" name="form_stats_nbClientsD" id="form_stats_nbClientsD" class="form_admin">
	<fieldset>
		<legend>Nombre de clients inscrits entre deux dates</legend>
		<p> <label for="input_stats_nbClientsD_dateB">Date de début :</label>
			<input type="text" name="input_stats_nbClientsD_dateB" required="required" placeholder="JJ/MM/AAAA" value="" id="input_stats_nbClientsD_dateB" />
		</p>
		<p> <label for="input_stats_nbClientsD_dateB">Date de fin :</label>
			<input type="text" name="input_stats_nbClientsD_dateE" required="required" placeholder="JJ/MM/AAAA" value="" id="input_stats_nbClientsD_dateE" />
		</p>
		<?php
		if (
			(isset($_POST['post_stats_nbClientsD'])) &&
			(isset($_POST['input_stats_nbClientsD_dateB']) && !empty($_POST['input_stats_nbClientsD_dateB'])) &&
			(isset($_POST['input_stats_nbClientsD_dateE']) && !empty($_POST['input_stats_nbClientsD_dateE']))
		){
			// On découpe la date dans un tableau, selon la forme JJ/MM/AAAA.
			$dateB = explode('/', $_POST['input_stats_nbClientsD_dateB']);
			$dateBOK = FALSE;
			if (count($dateB) == 3) {
				if (is_int(intval($dateB[1])) && is_int(intval($dateB[0])) && is_int(intval($dateB[2]))) {
					if (checkdate(intval($dateB[1]), intval($dateB[0]), intval($dateB[2]))) {
			...
			$dateE = explode('/', $_POST['input_stats_nbClientsD_dateE']);
			$dateEOK = FALSE;
			if (count($dateE) == 3) {
				if (is_int(intval($dateE[1])) && is_int(intval($dateE[0])) && is_int(intval($dateE[2]))) {
					if (checkdate(intval($dateE[1]), intval($dateE[0]), intval($dateE[2]))) {
			...
			if ($queryDB) {
				$id_shDB = quickConnectDB();
				if ($id_shDB != NULL) {
					$query = sprintf("SELECT * FROM `clients` WHERE `registeringDate` BETWEEN '%s' AND '%s';",
						mysql_real_escape_string($dateBq),
						mysql_real_escape_string($dateEq));
					$rep = mysql_query($query);
					$rows = array();
					...
					$nbClientsD = count($rows);
					mysql_close($id_shDB);
				}
				echo '<p>Nombre total de clients entre ' . $_POST['input_stats_nbClientsD_dateB'] . ' et ' . $_POST['input_stats_nbClientsD_dateE'] . ' : ' . $nbClientsD . '.</p>';
			}
		}
		?>
		<p> <input type="submit" value="Afficher" name="post_stats_nbClientsD" />
			<input type="reset" value="Réinitialiser" />
		</p>
	</fieldset>
</form>