<form method="get" action="admin.php" name="form_searchClientById" id="form_searchClientById" class="form_admin">
	...
	<p> <input type="hidden" name="action" value="modifyOverdraft">
	</p>
				...
				if (mysql_num_rows($rep) != 0) {
					...
					while ($result = mysql_fetch_array($rep)) {
						...
						if (mysql_num_rows($rep2) != 0) {
							...
							echo '<tr>';
							echo '<td><input type="checkbox" name="toModify[]" value="' . $result['id_account'] . '" /></td>';
							echo '<td>' . htmlentities($result2['name'], ENT_COMPAT, 'UTF-8') . '</td>';
							echo '<td>' . htmlentities($result['openingDate'], ENT_COMPAT, 'UTF-8'). '</td>';
							echo '<td><input type="text" name="' . $result['id_account'] . '" required="required" value="' . $result['overdraft'] . '" id="form_modifyOverdraft_overdraft' . $result['id_account'] . '" class="table_input"></td>';
							echo '</tr>';
						}
					}
					echo'
								...
								<p> <input type="hidden" name="idClient" value="' . $_GET['idClient'] . '" />
									<input type="submit" value="Modifier" name="post_modifyOverdraft" />
		...
		if ($queryDB) {
			...
			if ($id_shDB != null) {
				...
				if (mysql_num_rows($rep) != 0) {
					...
					while ($result = mysql_fetch_array($rep)) {
						echo '<tr>';
						echo '<td><a href="admin.php?action=modifyOverdraft&idClient=' . $result['id_client'] . '">' . $result['id_client'] . '</a>' . '</td><td>' . $result['lastName'] . '</td><td>' . $result['firstName'] . ' ' . $result['secondName'] . ' ' . $result['thirdName'] . '</td><td>' . $result['address_city'] . '</td>';
						echo '</tr>';
					}
					echo'
								</table>
...
if (isset($_POST['toModify']) && isset($_POST['idClient'])) {
	$id_shDB = quickConnectDB();
	if ($id_shDB != null) {
		$toModifyTab = $_POST['toModify']; 
		for ($i = 0; $i < count($toModifyTab); $i++) {
			if (!empty($toModifyTab[$i])) {
				if (ctype_digit($toModifyTab[$i])) {
					$query = sprintf("UPDATE `accounts` SET `overdraft` = '%s' WHERE `id_account` = '%s';",
					mysql_real_escape_string($_POST[$toModifyTab[$i]]),
					mysql_real_escape_string($toModifyTab[$i]));
					$validQuery = mysql_query($query);
					if ($validQuery) {
						echo showFormSuccess('Modification réussie, ID client : #' . htmlentities($_POST['idClient'], ENT_COMPAT, 'UTF-8') . ', ID contrat : #' . htmlentities($toModifyTab[$i], ENT_COMPAT, 'UTF-8'));
					}
				} else {
					echo showFormError('Modification', 'N\'insérez que des chiffres.');
				}
			} else {
				echo showFormError('Modification', 'Entrez des valeurs pour le(s) coompte(s).');
			}
		}
		mysql_close($id_shDB);
	}
